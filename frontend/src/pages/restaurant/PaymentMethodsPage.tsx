...
import { Banknote, CreditCard, Landmark, Pencil, Plus, Smartphone, Trash2, Wallet as WalletIcon } from 'lucide-react';
...
export default function PaymentMethodsPage() {
  ...
  const { data: methods, isLoading } = useQuery({
    ...
    queryFn: () => apiGet<PaymentMethodConfig[]>(endpoints.paymentMethods.list),
  });

  const toggleActive = useMutation({
    mutationFn: ({ id, isActive }: { id: number; isActive: boolean }) =>
      apiPatch(endpoints.paymentMethods.detail(id), { isActive }),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['payment-methods'] }),
    onError: (error) => toast.error(error instanceof ApiError ? error.message : 'Could not update that.'),
  });

  const deleteMethod = useMutation({
    mutationFn: (id: number) => apiDelete(endpoints.paymentMethods.detail(id)),
    onSuccess: () => {
  ...
  return (
    <div className="space-y-6">
      ...
      {modal.open && <MethodModal editing={modal.editing} onClose={() => setModal({ open: false })} />}
    </div>
  );
}

function MethodModal({ editing, onClose }: { editing?: PaymentMethodConfig; onClose: () => void }) {
  const queryClient = useQueryClient();
  ...
  const save = useMutation({
    mutationFn: (payload: Record<string, unknown>) =>
      editing
        ? apiPut(endpoints.paymentMethods.detail(editing.id), payload)
        : apiPost(endpoints.paymentMethods.create, payload),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['payment-methods'] });
